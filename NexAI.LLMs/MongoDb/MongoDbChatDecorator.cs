...
public class MongoDbChatDecorator(Chat chat, ConversationMongoDbCollection collection) : Chat
{
    ...
    public override async Task<string> Ask(ConversationId conversationId, string systemMessage, string message, CancellationToken cancellationToken)
    {
        ...
        await UpsertConversationIfCurrent(conversationId, ToMessages(systemMessage, message), response, cancellationToken);
        return response;
    }
    ...
    public override async Task<TResponse> Ask<TResponse>(ConversationId conversationId, string systemMessage, string message, CancellationToken cancellationToken)
    {
        ...
        await UpsertConversationIfCurrent(conversationId, ToMessages(systemMessage, message), response.ToString() ?? string.Empty, cancellationToken);
        return response;
    }

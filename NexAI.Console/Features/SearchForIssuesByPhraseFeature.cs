...
public class SearchForIssuesByPhraseFeature(Options options)
{
    public async Task Run(ulong limit)
    {
        while (true)
        {
            ...
            try
            {
                var similarIssues = await new FindSimilarIssuesByPhraseQuery(options).Handle(userMessage, limit);
